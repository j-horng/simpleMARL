...
#Discrete Action Critic
class Critic(nn.Module):
    def __init__(self, full_state_space, config=PPOConfig()):
        super().__init__()
        self.config = config
        self.optimizer = None
        self.critic = nn.Sequential(
            layer_init(nn.Linear(np.array(full_state_space.shape).prod(), 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64, 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64,1), std=1.0),
    ...
    def anneal_lr(self, iteration):
        if self.config.anneal_lr:
            ...
            self.optimizer.param_groups[0]["lr"] = lrnow 
    def get_action_and_value(self, x, action=None):
        return None, None, None, self.critic(x)
    def get_value(self, x):
        return self.critic(x)
    def save(self, path=f"ppoCritic_{time.time()}.pt"):
        torch.save(self.state_dict(), path)
    def load(self, path=None):
        assert path != None, "Error: Missing Argument 'path'; Cannot load a model without a path"
        return self.load_state_dict(torch.load(path))
    def update(self, mini_batch):
        ...
        for k in mini_batch:
            if k == 'actions':
                ...
                mini_batch[k] = mini_batch[k].to(self.config.device)
        print("Kyes: ", mini_batch.keys())
        newvalue = self.get_value(mini_batch['states'])
        # Value Loss
        newvalue = newvalue.view(-1)
        if self.config.clip_vloss:
            v_loss_unclipped = (newvalue - mini_batch['returns']) **2
        ...
        nn.utils.clip_grad_norm_(self.parameters(), self.config.max_grad_norm)
        # self.optimizer.step()
        logs = {"v_loss":v_loss.item(), "pg_loss":None, "entropy_loss":None, "old_approx_kl":None, "approx_kl":None, "clipfracs":np.mean(clipfracs)}
        return logs
...
class Actor(nn.Module):
    ...
    def save(self, path=f"ppoActor_{time.time()}.pt"):
        torch.save(self.state_dict(), path)
